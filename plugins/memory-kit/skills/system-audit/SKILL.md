...
- A finding you could not verify stays in the report marked `гипотеза` + the check that would settle it. Never silently dropped, never promoted to fact.
...
- **Don't report anything you didn't verify** as fact. `гипотеза` is an honest label; a confident
