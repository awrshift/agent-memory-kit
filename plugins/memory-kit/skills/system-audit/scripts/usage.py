...
Manual invocation:
    python3 <plugin>/skills/system-audit/scripts/usage.py   (lens 4 of /memory-kit:system-audit)
"""
...
# v6: the script ships inside the plugin, so its depth on disk says nothing about the
# repository being measured — the project root comes from the harness. (Deriving it from
# __file__ made the script report on the PLUGIN's own transcripts, silently.)
...
def render_report(stats: dict, now: datetime, total_events: int, after_burst: int) -> str:
    ...
    lines += [
        ...
        "---",
        "",
        "*Generated by `/memory-kit:system-audit` (layer-telemetry lens). Read-only.*",
        "",
    ]
    return "\n".join(lines)
