...
`/memory-kit:close-session` with 2-3 sentences of where things stand. This header is «current state»,
...
Every entry is `[YYYY-MM-DD]`-prefixed. This is what lets the `/memory-kit:close-session` audit work —
...
densify into ever-longer lines while `wc -l` stays flat. When a cap trips, `/memory-kit:close-session`
